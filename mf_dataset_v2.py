...
class MaterialistFashion(data.Dataset):
    ...
    def __init__(self, image_folder, json_file, transform=None, percent=100, load_first=-1):
        ...
        with open(json_file, 'r') as f:
            ...
            self.annotations = self.mlb.fit_transform(mf_data['labelId']).astype(np.float32)
            x, y = self.annotations.shape
            if y != 228:
                to_concat = 228-y
                self.annotations = np.concatenate((self.annotations, np.zeros((x, to_concat))), axis=1)

            self.image_id = list(mf_data['imageId'])
            if percent < 100:
                temp_annotations = []
                temp_image_id = []
    ...
    def __getitem__(self, index):
        image_data = self.default_loader(os.path.join(self.image_folder, str(self.image_id[index]) + '.jpg'))
        if self.transform:
